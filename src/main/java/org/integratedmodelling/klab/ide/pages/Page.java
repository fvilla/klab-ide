...
import org.integratedmodelling.klab.ide.events.EventBus;
...
public interface Page {
  ...
  int MAX_WIDTH = KlabIDEApplication.MIN_WIDTH - KlabIDEApplication.SIDEBAR_WIDTH;
  int HGAP_20 = 20;
  int HGAP_30 = 30;
  int VGAP_10 = 10;
  int VGAP_20 = 20;
      ...
      node.addEventFilter(
          ...
          e -> {
            if (e.getTarget() instanceof Hyperlink link
                && link.getUserData() instanceof String url) {
              if (url.startsWith("https://") || url.startsWith("http://")) {
                EventBus.getInstance().publish(new EventBus.BrowseEvent(URI.create(url)));
              }
                ...
                try {
                  ...
                  if (Page.class.isAssignableFrom(c)) {
                    EventBus.getInstance()
                        .publish(new EventBus.NavEvent((Class<? extends Page>) c));
                  } else {
